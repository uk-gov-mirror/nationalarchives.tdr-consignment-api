...
import akka.http.scaladsl.model.{ContentType, ContentTypes, MediaType, StatusCodes}
import akka.http.scaladsl.server.Directives.{as, authenticateOAuth2Async, complete, entity, get, getFromResource, path, post}
...
object Routes {
  ...
  val route: Route =
    (post & path("graphql")) {
      ...
      getFromResource("schema.graphql", ContentTypes.`text/plain(UTF-8)`)
    }
}
